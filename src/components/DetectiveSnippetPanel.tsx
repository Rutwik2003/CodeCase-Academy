...
interface DetectiveSnippetPanelProps {
  ...
  isVisible: boolean;
  onToggle: () => void;
  availableHints: number; // Number of hints the user has
}
...
export const DetectiveSnippetPanel: React.FC<DetectiveSnippetPanelProps> = ({
  ...
  const handleInsertSnippet = (snippet: DetectiveSnippet) => {
    ...
    if (availableHints <= 0) {
      // Show restriction message instead of inserting snippet
      return;
    }
  ...
  if (!isVisible) {
    ...
    const isRestricted = availableHints <= 0;
    
    return (
      <div className="fixed bottom-4 right-4 z-50">
        <motion.button
          onClick={onToggle}
          whileHover={{ scale: isRestricted ? 1 : 1.1 }}
          whileTap={{ scale: isRestricted ? 1 : 0.9 }}
          className={`${
            isRestricted 
              ? 'bg-gray-400 cursor-not-allowed' 
              : 'bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700'
          } text-white p-3 rounded-full shadow-lg relative`}
        ...
        {isRestricted && (
          <motion.div
            ...
            className="absolute bottom-16 right-0 bg-red-500 text-white text-xs p-2 rounded-lg whitespace-nowrap"
          >
            No hints available! Sign in to get more.
          </motion.div>
        )}
      </div>
    );
    ...
    >
      ...
      {availableHints <= 0 && (
        <div className="absolute inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center rounded-xl">
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg text-center max-w-xs">
            <div className="text-4xl mb-3">🔒</div>
            <h3 className="font-bold text-lg mb-2 theme-text-primary">No Hints Available</h3>
            <p className="text-sm theme-text-muted mb-4">
              You need hints to access the Detective Toolkit. Sign in to get more hints and unlock all features!
            </p>
            <button
              onClick={onToggle}
